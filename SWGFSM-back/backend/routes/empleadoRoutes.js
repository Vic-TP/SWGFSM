...
router.post("/login", async (req, res) => {
  try {
    ...
    res.json({
      empleado: toPublicEmpleado(empleado),
      token: token,
    });
  } catch (err) {
    console.error(err);
...
router.post("/", async (req, res) => {
  try {
    ...
    if (existe) {
      return res
        .status(409)
        .json({ message: "Ya existe un empleado con ese correo." });
    }
  ...
  try {
    ...
    if (correo !== empleado.correo) {
      ...
      if (otro) {
        return res
          .status(409)
          .json({ message: "Ya existe otro empleado con ese correo." });
      }
    }

    empleado.nombres = String(
      b.nombres != null ? b.nombres : empleado.nombres,
    ).trim();
