...
const ACCESS_TOKEN = "APP_USR-8369515796622368-061822-3fb1c3bfd0d625e087ea6e607c314851-2428020760";
...
router.post("/create_preference", async (req, res) => {
  ...
  if (!token || !payment_method_id || !transaction_amount || !payer?.email) {
    return res.status(400).json({ 
      message: "Faltan datos requeridos: token, payment_method_id, transaction_amount o email del pagador." 
    });
  }
  ...
  try {
    ...
    console.error("Error MercadoPago:", JSON.stringify(error, null, 2));
    res.status(500).json({ 
      message: error?.cause?.[0]?.description || "Error al procesar el pago." 
    });
  }
});
