      ...
      if (invRes.ok) {
        const mapped = invRes.data.map((p) => ({
          ...
          producto: (() => {
            ...
            const base =
              p.producto === "Palta" ? `Palta ${p.tipo}` : p.producto;
  ...
  return (
    ...
    >
      <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden border border-red-200">
        <div
          className={`px-6 py-5 flex items-start gap-4 ${tienePuntoNegro ? "bg-red-50" : "bg-yellow-50"}`}
        >
          <div
            className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shrink-0 ${tienePuntoNegro ? "bg-red-100" : "bg-yellow-100"}`}
          >
            {tienePuntoNegro ? "⚠️" : "🟡"}
          </div>
          <div>
            <p
              className={`font-semibold text-sm ${tienePuntoNegro ? "text-red-800" : "text-yellow-800"}`}
            >
              {tienePuntoNegro
                ? "¡Paltas en punto negro detectadas!"
                : "Paltas maduras — acción requerida"}
            </p>
            <p
              className={`text-xs mt-0.5 ${tienePuntoNegro ? "text-red-600" : "text-yellow-600"}`}
            >
              El modelo ML identificó {criticos.length} lote
              {criticos.length > 1 ? "s" : ""} crítico
              {criticos.length > 1 ? "s" : ""}. Actúa de inmediato.
            </p>
          </div>
        </div>
        <div className="px-6 py-4 space-y-3">
          {criticos.map((p, i) => (
            <div
              key={i}
              className={`rounded-2xl border p-3 flex items-start justify-between gap-3 ${p.estadoML === "punto_negro" ? "bg-red-50 border-red-100" : "bg-yellow-50 border-yellow-100"}`}
            >
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  {p.cant} kg · {p.diasAlmacen} días almacenado · {p.proveedor}
                </p>
              </div>
              <div className="shrink-0 text-right">
                <span
                  className={`inline-block px-2 py-0.5 rounded-full text-[11px] font-semibold ${p.estadoML === "punto_negro" ? "bg-red-100 text-red-700" : "bg-yellow-100 text-yellow-700"}`}
                >
                  {ESTADO_CONFIG[p.estadoML].label}
                </span>
                <p
                  className={`text-[11px] font-semibold mt-1 ${p.estadoML === "punto_negro" ? "text-red-600" : "text-yellow-600"}`}
                >
        ...
        <div className="px-6 pb-5 flex gap-3">
          <button
            onClick={onDismiss}
            className="flex-1 py-2.5 rounded-xl text-sm font-semibold bg-emerald-700 text-white hover:bg-emerald-800 transition"
          >
...
const Badge = ({ estado }) => {
  ...
  return (
    <span
      className={`inline-block px-3 py-0.5 rounded-full text-xs font-semibold ${cfg.bg} ${cfg.text}`}
    >
      {cfg.label}
    </span>
  );
...
const PhBadge = ({ ph }) => (
  <span className="inline-block px-2 py-0.5 rounded-full text-[11px] font-semibold bg-sky-100 text-sky-800 font-mono">
    {ph}
  </span>
);
const CalBadge = ({ cal }) => {
  const map = {
    ...
    Bueno: "bg-sky-100 text-sky-800",
    Regular: "bg-yellow-100 text-yellow-800",
  };
  return (
    <span
      className={`inline-block px-3 py-0.5 rounded-full text-xs font-semibold ${map[cal] ?? "bg-gray-100 text-gray-700"}`}
    >
      {cal}
    </span>
  );
...
const DotTimeline = ({ color }) => {
  ...
  return (
    <span
      className={`absolute -left-[9px] top-1 w-3 h-3 rounded-full border-2 border-lime-50 ${map[color]}`}
    />
  );
};
const HBar = ({ label, value, max, color }) => {
  ...
  return (
    <div className="flex items-center gap-3 text-xs">
      <span className="w-24 text-right text-emerald-700 shrink-0 font-medium">
        {label}
      </span>
      <div className="flex-1 h-4 bg-lime-100 rounded overflow-hidden">
        {value > 0 ? (
          <div
            className={`h-full ${color} flex items-center pl-2 text-white font-semibold text-[10px]`}
            style={{ width: `${Math.max(pct, 6)}%` }}
          >
            {value} kg
          </div>
        ) : (
          <div className="h-full flex items-center pl-2 text-gray-400 text-[10px]">
            0 kg
          </div>
        )}
      </div>
      <span
        className={`w-10 text-right font-semibold ${value > 0 ? "text-emerald-700" : "text-gray-400"}`}
      >
        {pct}%
      </span>
    </div>
...
const Skel = ({ className }) => (
  <div className={`bg-lime-200 animate-pulse rounded-lg ${className}`} />
);
const ChartSkeleton = ({ height = "h-44" }) => (
  <div
    className={`${height} flex flex-col items-center justify-center gap-2 bg-lime-50 rounded-xl border border-lime-200`}
  >
...
const InfoNote = ({ children }) => (
  <p className="text-xs text-emerald-600 mt-2 px-3 py-2 bg-lime-50 rounded-lg border border-lime-200">
    {children}
  </p>
);
const ConfBar = ({ value }) => (
  <div className="flex items-center gap-2">
    <div className="flex-1 h-1.5 bg-lime-100 rounded-full overflow-hidden w-16">
      <div
...
const diasColor = (d) =>
  d >= 7 ? "text-red-700" : d >= 5 ? "text-yellow-700" : "text-emerald-700";
const accionColor = (e) =>
  e === "punto_negro" || e === "maduro" ? "text-red-600" : "text-emerald-700";
const accionPrefix = (e) =>
  e === "punto_negro" || e === "maduro" ? "⚠ " : "✓ ";
...
const LineChartCanvas = ({ enRiesgo, enSazon }) => {
  ...
  const gen = (end, f) =>
    Array.from({ length: 7 }, (_, i) =>
      Math.max(0, Math.round(end * (0.4 + (i / 6) * 0.6 * f))),
    );
  useChart(ref, {
    type: "line",
    data: {
  ...
  useChart(ref, {
    ...
    data: {
      ...
      datasets: [
        {
          data,
          backgroundColor: ["#dc2626", "#d97706", "#15803d", "#0284c7"],
          borderWidth: 0,
        },
      ],
    },
...
const Prediccion = () => {
  ...
  const reentrenarBackend = useCallback(async () => {
    try {
      await fetchWithAuth(`${API}/reentrenar`, { method: "POST" });
  ...
  const totalKg =
    resumen?.totalKg ?? predictions.reduce((s, p) => s + (p.cant ?? 0), 0);
  const totalRiesgo =
    resumen?.totalRiesgo ??
    predictions.filter(
      (p) => p.estadoML === "punto_negro" || p.estadoML === "maduro",
  ...
  const totalSazon =
    resumen?.totalSazon ??
    predictions.filter((p) => p.estadoML === "sazon").length;
  const diasProm =
    resumen?.diasProm ??
    (predictions.length
      ? (
          predictions.reduce((s, p) => s + (p.diasAlmacen ?? 0), 0) /
          predictions.length
        ).toFixed(1)
      : "—");
  const avgConf =
    ...
    (predictions.length
      ? Math.round(
          (predictions.reduce((s, p) => s + (p.confianza ?? 0), 0) /
            predictions.length) *
            100,
        )
      : null);
  ...
  const doughnutData = ["punto_negro", "maduro", "sazon", "verde"].map(
    countByEstado,
  );
  ...
  const barValues = PROVEEDORES.map((p) =>
    mlStatus === "ready" ? p.riesgo : 0,
  );
  ...
  const productosFiltrados = predictions.filter((p) => {
    if (filtro === "riesgo")
      return p.estadoML === "punto_negro" || p.estadoML === "maduro";
  ...
  return (
    <>
      {showAlert && (
        <AlertModal predictions={predictions} onDismiss={handleDismissAlert} />
      )}
          ...
          <div className="px-8 pt-6 pb-4 border-b border-lime-200 flex flex-wrap items-start justify-between gap-3">
            <div>
              <h2 className="text-lg font-semibold text-emerald-900">
                Predicción de madurez — Paltas
              </h2>
              <p className="text-xs text-emerald-700 mt-0.5">
                Actualizado: {new Date().toLocaleDateString("es-PE")} · Lote
                actual: {totalKg} kg
              </p>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {mlStatus === "ready" && totalRiesgo > 0 && (
                ...
                >
                  ⚠ {totalRiesgo} lote{totalRiesgo > 1 ? "s" : ""} en riesgo —
                  ver alerta
                </button>
              )}
              <button
          ...
          <div className="p-6 lg:p-8">
            ...
            {tab === "resumen" && (
              <div className="space-y-6">
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  ...
                  />
                  <MetricCard
                    label="Días prom. almacén"
                    value={diasProm}
                    sub="Calculado desde fechas"
                  ...
                  {mlStatus === "ready" ? (
                    <MetricCard
                      ...
                      value={totalRiesgo}
                      sub="Detectados por ML"
                      valueColor="text-red-600"
                    />
                  ) : (
                    <div className="bg-lime-100 rounded-2xl p-4 space-y-2">
                  ...
                  {mlStatus === "ready" ? (
                    <MetricCard
                      label="En sazón"
                      value={totalSazon}
                      sub="Listos para venta"
                      valueColor="text-emerald-700"
                    />
                  ) : (
                    <div className="bg-lime-100 rounded-2xl p-4 space-y-2">
                      <p className="text-xs text-emerald-700">En sazón</p>
                      <Skel className="h-8 w-16" />
                      <Skel className="h-3 w-24" />
                    </div>
                  )}
                ...
                <div className="grid lg:grid-cols-2 gap-6">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700 mb-3">
                      Predicciones ML — lotes actuales
                    </p>
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {predictions.map((p, i) => (
                            <tr
                              key={i}
                              className="border-t border-lime-100 hover:bg-lime-100"
                            >
                              <td className="px-4 py-2.5">
                                ...
                                <p className="text-[11px] text-emerald-600">
                                  {p.cant} kg · {p.proveedor}
                                </p>
                              </td>
                              <td
                                className={`px-4 py-2.5 text-xs font-semibold ${diasColor(p.diasAlmacen)}`}
                              >
                                {p.diasAlmacen}d
                              </td>
                              <td className="px-4 py-2.5">
                              ...
                              <td
                                className={`px-4 py-2.5 text-xs font-semibold ${accionColor(p.estadoML)}`}
                              >
                                {accionPrefix(p.estadoML)}
                                {
                                  ACCIONES_MAP[
                                    ESTADO_LABELS.indexOf(p.estadoML)
                                  ]
                                }
                              </td>
                            </tr>
                    ...
                    <InfoNote>
                      Estado y acción calculados por el modelo ML del backend
                    </InfoNote>
                  </div>
                  <div className="space-y-5">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700 mb-3">
                        Distribución de kg por estado
                      </p>
                      {mlStatus === "ready" ? (
                        <div className="space-y-2">
                          {["punto_negro", "maduro", "sazon", "verde"].map(
                            (est, i) => {
                              const colors = [
                                  "bg-red-500",
                                  "bg-yellow-500",
                    ...
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700 mb-3">
                        Vida útil por lote (días desde ingreso)
                      </p>
                      <div className="space-y-2.5">
                        {predictions.slice(0, 4).map((item) => {
                          ...
                          const ph =
                            item.phEstimado ??
                            estimatePh(dias, item.tamano, item.tipo);
                          const color =
                            dias >= 7
                              ? "bg-red-500"
                              : dias >= 5
                          ...
                          return (
                            <div
                              key={item._id || `${item.producto}-${item.fecha}`}
                            >
                              <div className="flex justify-between text-xs mb-0.5">
                                <span className="text-emerald-800">
                                  {item.producto}
                                </span>
                                <span className={diasColor(dias)}>
                                  {dias}d · pH {ph}
                                </span>
                              </div>
                              <ProgressBar
              ...
              <div className="space-y-4">
                <div className="flex gap-2 flex-wrap">
                  {[
                    ...
                    { id: "sazon", label: "En sazón" },
                  ].map((f) => (
                    <button
                      key={f.id}
                      onClick={() => mlStatus === "ready" && setFiltro(f.id)}
                    ...
                    <tbody>
                      ...
                      {productosFiltrados.map((p, i) => {
                        ...
                        const phE =
                          p.phEstimado ??
                          (dias != null
                            ? estimatePh(dias, p.tamano, p.tipo)
                            : null);
                        return (
                          <tr
                            key={i}
                          ...
                          >
                            ...
                            <td
                              className={`px-4 py-2.5 text-xs font-semibold ${dias != null ? diasColor(dias) : "text-gray-400"}`}
                            >
                              {dias != null ? `${dias}d` : "—"}
                            </td>
                            <td className="px-4 py-2.5">
                ...
                <InfoNote>
                  Estado y Acción determinados por ML del backend · pH estimado
                  desde días, tamaño y tipo · unidades en kg
                </InfoNote>
              </div>
            )}
              ...
              <div className="space-y-6">
                  ...
                  {mlStatus === "ready" ? (
                    <>
                      <MetricCard
                        ...
                        sub="Reportada por backend"
                        valueColor="text-emerald-700"
                      />
                      <MetricCard
                        label="Confianza media"
                        value={avgConf != null ? `${avgConf}%` : "—"}
                        sub="En todos los lotes"
                        valueColor="text-sky-700"
                      />
                      <MetricCard
                        label="En riesgo detectados"
                        value={totalRiesgo}
                        sub="Punto negro + maduro"
                        valueColor="text-red-600"
                      />
                    </>
                  ) : (
                    [1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="bg-lime-100 rounded-2xl p-4 space-y-2"
                      >
                        <Skel className="h-3 w-24" />
                        <Skel className="h-8 w-16" />
                ...
                {chartReady ? (
                  <div className="space-y-4">
                    <div className="grid lg:grid-cols-2 gap-4">
                      <div className="rounded-2xl border border-lime-200 p-4">
                        <p className="text-xs font-semibold text-emerald-800 mb-1">
                          Evolución estimada — 7 días
                        </p>
                        <p className="text-[10px] text-gray-400 mb-3">
                          Proyección basada en estado ML actual
                        </p>
                        {mlStatus === "ready" ? (
                          <div className="relative h-44">
                            <LineChartCanvas
                              key={`l-${totalRiesgo}-${totalSazon}`}
                              enRiesgo={predictions
                                .filter(
                                  (p) =>
                                    p.estadoML === "punto_negro" ||
                                    p.estadoML === "maduro",
                      ...
                      <div className="rounded-2xl border border-lime-200 p-4">
                        <p className="text-xs font-semibold text-emerald-800 mb-3">
                          Distribución por estado — kg (ML)
                        </p>
                        {mlStatus === "ready" ? (
                          <div className="relative h-44">
                            <DoughnutChartCanvas
                              key={`d-${doughnutData.join("-")}`}
                              data={doughnutData}
                            />
                          </div>
                        ) : (
                    ...
                    <div className="rounded-2xl border border-lime-200 p-4">
                      <p className="text-xs font-semibold text-emerald-800 mb-3">
                        Kg en riesgo por proveedor (ML)
                      </p>
                      {mlStatus === "ready" ? (
                        <div className="relative h-36">
                          <BarChartCanvas
                            key={`b-${barValues.join("-")}`}
                            labels={barLabels}
                            values={barValues}
                          />
                        </div>
            ...
            {tab === "inventario" && (
              <div className="space-y-4">
                    ...
                    <tbody>
                      {predictions.map((p, i) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-2.5 text-xs font-semibold text-emerald-800">
                            {p.cant} kg
                          </td>
                          <td
                            className={`px-4 py-2.5 text-xs font-semibold ${diasColor(p.diasAlmacen)}`}
                          ...
                          <td className="px-4 py-2.5">
                            <PhBadge
                              ph={
                                p.phEstimado ??
                                estimatePh(p.diasAlmacen ?? 0, p.tamano, p.tipo)
                              }
                            />
                          </td>
                          <td className="px-4 py-2.5">
                ...
                <InfoNote>
                  Días calculados en tiempo real desde fecha de ingreso · pH
                  desde backend o estimado · Estado y Acción por ML
                </InfoNote>

                {mlStatus === "ready" && PROVEEDORES.length > 0 && (
                  <div className="pt-2">
                    <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700 mb-3">
                      Calificación por proveedor (ML)
                    </p>
                    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                      {PROVEEDORES.map((pv) => (
                        <div
                          key={pv.nombre}
                          className="rounded-2xl border border-lime-200 p-4 bg-white space-y-1.5"
                        >
                          <p className="font-semibold text-sm text-emerald-900">
                            {pv.nombre}
                          </p>
                          <div className="flex justify-between text-xs">
                            <span className="text-gray-500">Kg total</span>
                          ...
                          <div className="flex justify-between text-xs">
                            ...
                            <span
                              className={`font-medium ${pv.riesgo > 0 ? "text-red-600" : "text-emerald-700"}`}
                            >
                              {pv.riesgo} kg
                            </span>
                          </div>
                          <ProgressBar
                            ...
                            max={pv.unidades || 1}
                            color={
                              pv.riesgo > 0 ? "bg-red-500" : "bg-emerald-500"
                            }
                          />
            ...
            {tab === "historial" && (
              <div className="space-y-6">
                {mlStatus === "ready" && predictions.length > 0 && (
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700 mb-3">
                      Alertas generadas por el modelo — actuales
                    </p>
                    <div className="rounded-2xl border border-lime-200 overflow-hidden">
                      <table className="w-full text-sm">
                        <thead className="bg-emerald-900 text-lime-50">
                        ...
                        <tbody>
                          {[...predictions]
                            .sort(
                              (a, b) =>
                                ESTADO_LABELS.indexOf(b.estadoML) -
                                ESTADO_LABELS.indexOf(a.estadoML),
                            )
                            .map((p, i) => (
                              <tr
                                key={i}
                ...
                {historialFuente.length > 0 && (
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700 mb-3">
                      Línea de tiempo — eventos registrados
                    </p>
                    <div className="relative pl-5 border-l-2 border-lime-200 space-y-5">
                      {historialFuente.map((ev, i) => (
                        <div key={i} className="relative pl-3">
                          ...
                          <span
                            className={`inline-block mt-1 px-2 py-0.5 rounded-full text-[10px] font-semibold ${ev.color === "rojo" ? "bg-red-100 text-red-700" : ev.color === "amarillo" ? "bg-yellow-100 text-yellow-700" : "bg-emerald-100 text-emerald-700"}`}
                          >
                            {ev.fecha}
                          </span>
                        </div>
                ...
                <div className="pt-2 flex items-center gap-4">
                  ...
                  <p className="text-[11px] text-gray-400">
                    Tarda ~30 s · los resultados se recargan automáticamente
                  </p>
                </div>
              </div>
            )}
